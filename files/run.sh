...
CA_CERT="${HAPROXY_FOLDER}/root_ca.crt"
...
/bin/cat "${TLS_CERT}" "${CA_CERT}" "${TLS_KEY}" > "${TLS_CHAIN}"
...
inotifywait -m "${HAPROXY_CFG}" "${TLS_CERT}" "${CA_CERT}" "${TLS_KEY}" -e create,delete,modify,attrib |
  while read path; do
    /bin/cat "${TLS_CERT}" "${CA_CERT}" "${TLS_KEY}" > "${TLS_CHAIN}"
